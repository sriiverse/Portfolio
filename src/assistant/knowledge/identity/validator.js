/**
 * identity/validator.js — Domain validator for category "identity".
 */
...
export function validateDocument(doc) {
  ...
  if (doc.category !== 'identity') {
    return {
      ok: false,
      errors: [{ path: '$.category', message: 'expected category "identity"' }],
    };
  }
...
export function validateDocuments(docs) {
  ...
  for (const doc of list) {
    ...
    if (!result.ok) {
      ...
      throw new Error(`[identity/validator] ${doc?.id || '?'} — ${detail}`);
    }
  }
  return list;
}
...
export function assertDocument(doc) {
  assertValid(doc, schema, 'identity document');
  return doc;
}
